function options = LTS_settings(varargin)
%% LTS_xxx
% AUTHOR:
...
% 
% FUNCTION: 
% Both general and solver-specific settings are selected here
% 
% SYNTAX
% When giving one input with varargin, e.g. with settings(20), will use h-method of your choice with n nodes
% When giving two inputs with varargin, hp-LGR method will be used with two possibilities
% - Scalar numbers can be used for equally spaced intervals with the same polynoimial orders. For example, 
% ...settings_hp(5,4) means using 5 LGR intervals each of polynomial degree of 4. 
% - Alternatively, can supply two arrays in the argument with customized meshing. For example, 
% ...settings_hp([-1 0.3 0.4 1],[4 5 3]) will have 3 segments on the normalized interval [-1 0.3], [0.3 0.4] and [0.4 1],
% ...with polynomial order of 4, 5, and 3 respectively.
% 
% INPUTS:
% (see SYNTAX)
%
...
options.adigatorPath='C:\Users\Kreisler\Documents\MATLAB\matt-weinstein-adigator-47059cc'; %'../../adigator';
...
options.ECH.enabled=1; 
% A conservative buffer zone (e.g. set it to 0.1, i.e. 10%)
...
if nargin==2
    if strcmp(varargin{2},'h')
        ...
        disp("AUTOMATICALLY SWITCHING TO hpLGR!")
        pause(2)
        options.discretization='hpLGR';
%         options.discretization='globalLGR';
    end
else
    options.nodes=varargin{1}; 
end
...
% Plot track-related figures
% 0: Do not plot
% 1: Plot all figures
options.plot_track=1;
