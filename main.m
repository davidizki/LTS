...
set(0,'DefaultFigureVisible','off') % set(0,'DefaultFigureWindowState','minimized')
...
[tv,xv,uv] = simulateSolution(problem,solution,'ode113'); % Check the model in open loop


%% 2. POST-PROCESSING
% 1. Default Plots
genSolutionPlots(options, solution); % Built-in figure generation (check settings for the type of plots to be generated)
h = findobj('type','figure'); % Useful to get the # figures

% 2. Summary Messages
fprintf('SUMMARY: %.2f s - TIME (Total)\n',sum(MRHistory.timeHistory));
fprintf('SUMMARY: %d - # ITERATIONS (Refinement)\n',length(MRHistory.iterHistory));
fprintf('SUMMARY: %d - # EVALUATIONS (of Lagrangian Hessian)\n',sum(MRHistory.iterHistory));

% 3.a Plot-Variables (plotvar)
...
% 3.b Plot-Parameters (plotparam)
plotparam.Nfigures = length(h); % # figures generated by default function. Variable to control the generation of figures
